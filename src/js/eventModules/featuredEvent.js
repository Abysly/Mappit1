export async function loadFeaturedEvents() {
  try {
    ...
    events.forEach((event) => {
      ...
      el.innerHTML = `
  <div class="event-card bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-300 group h-full flex flex-col">
    <!-- Header with icon and basic info -->
    <div class="p-5 flex items-start gap-4">
      <div class="w-16 h-16 bg-gradient-to-br from-blue-100 to-blue-50 text-blue-600 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm">
    ...
    <div class="border-t border-gray-100 p-5 mt-auto">
    ...
    ${
      event.isSpecial
        ? `<div class="absolute top-3 right-3 bg-red-500 text-white px-2 py-1 rounded-md text-xs font-semibold transform rotate-6">Featured</div>`
        : ""
    }
  </div>
`;
